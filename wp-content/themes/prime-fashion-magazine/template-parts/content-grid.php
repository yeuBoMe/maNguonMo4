	...
	<?php } ?>
	 ...
	 if ( $prime_fashion_magazine_image_setting ){ 
		 echo ( !is_single() ) ? '<a href="' . esc_url( get_the_permalink() ) . '" class="post-thumbnail">' : '<div class="post-thumbnail">'; ?>
	 			<?php ( is_active_sidebar( 'right-sidebar' ) ) ? the_post_thumbnail( 'prime-fashion-magazine-with-sidebar', array( 'itemprop' => 'image' ) ) : the_post_thumbnail( 'prime-fashion-magazine-without-sidebar', array( 'itemprop' => 'image' ) ) ; ?>
	    <?php echo ( !is_single() ) ? '</a>' : '</div>' ;?>
	 <?php } ?>
    <?php
